...
router.get("/getImageUrl", (req, res) => {
    ...
    let finalData
    let resArr = []
    var files = fs.readdirSync('./UploadImage/')
    files.forEach(function (item, index) {
        var stream = fs.createReadStream('./UploadImage/' + item)
        //存储文件流
        if (stream) {  //判断状态
...
router.post("/saveImageUrl", (req, res) => {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        console.log(files);
        const newPath = form.uploadDir + '\\UploadImage' + '\\' + fileName
